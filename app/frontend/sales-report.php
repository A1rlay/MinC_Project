...
<div class="professional-card rounded-xl p-6 mb-6">
    ...
    <form method="GET" class="flex flex-col md:flex-row gap-4 items-end">
        <div class="flex-1">
            ...
            <input type="date" name="start_date" value="<?= htmlspecialchars($start_date) ?>" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#08415c] focus:border-transparent">
        </div>
        <div class="flex-1">
            <label class="block text-sm font-medium text-gray-700 mb-2">End Date</label>
            <input type="date" name="end_date" value="<?= htmlspecialchars($end_date) ?>" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#08415c] focus:border-transparent">
        </div>
        <button type="submit" class="filter-btn px-6 py-2 bg-gradient-to-r from-[#08415c] to-[#0a5273] text-white rounded-lg hover:shadow-lg transition">
            Generate Report
        </button>
    </form>
</div>
    ...
    <div class="professional-card rounded-xl p-4">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl font-bold text-green-600">₱<?= number_format($summary['total_revenue'] ?? 0, 0) ?></p>
            </div>
    ...
    <div class="professional-card rounded-xl p-4">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl font-bold text-blue-600">₱<?= number_format($summary['avg_order_value'] ?? 0, 2) ?></p>
            </div>
                ...
                <?php if (!empty($top_products)): ?>
                    <?php foreach ($top_products as $product): ?>
                        <tr class="report-row hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 text-sm font-semibold text-[#08415c]">₱<?= number_format($product['total_revenue'], 2) ?></td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr>
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                ?>
                    <tr class="report-row hover:bg-gray-50 transition">
                        ...
                        <td class="px-6 py-4 text-sm font-semibold text-[#08415c]">₱<?= number_format($cat['total_revenue'], 2) ?></td>
